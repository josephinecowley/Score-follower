...
wav_file = '/Users/josephine/Documents/Engineering /Part IIB/Score alignment project/Score-follower/wav_files/piano_A4_440.wav'
sample_rate, data = wav.read(wav_file)
data = data[5000:6000]
time_samples = np.linspace(
    0, len(data)/sample_rate, len(data))
helper.plot_audio(time_samples, data)
plt.show()
# probs = []
# frequencies = np.arange(20, 700, 5)
# for f in frequencies:
#     probs.append(-helper.stable_nlml(time_samples,
#                  data, f=[f], T=2, sigma_n=1e-3, M=12))
# print(probs)
# plt.plot(frequencies, probs)
# plt.show()
print(-helper.stable_nlml(time_samples, data, f=[440]))
...
#         f=beethoven_freqs[i], show=True, M=14, scalar=200, sigma_f=20, T=0.465, v=2.37)
