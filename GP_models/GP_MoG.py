...
def posterior(X_s, X_train, Y_train, M=1, sigma=0.1,  sigma_y=1e-8): 
    ...
    K_ss = plot_cov_matrix(X_s, X_s,M=M, sigma = sigma) + 1e-8 * np.eye(len(X_s))
    K_inv = inv(K)
...
mu_s, cov_s = posterior(X, X_train,Y_train, M = 3, sigma=0.1, sigma_y=noise)
...
mu_s, cov_s = posterior(X, X_train, Y_train, M=int(m), sigma=sigma_opt, sigma_y=noise)
