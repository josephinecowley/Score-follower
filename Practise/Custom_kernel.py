...
class MogKernel(Kern):
    ...
    def K(self, X, X2=None):
        ...
        exp_dist = np.exp(-0.5 * np.sum(np.square(np.sin(base) /
                          self.lengthscale), axis=-1))
    ...
    def gradients_X(self, dL_dK, X, X2=None):
        ...
        if X2 is None:
            dL_dK = dL_dK+dL_dK.T
            X2 = X
        dX = -np.pi*((dL_dK*K)[:, :, None]*np.sin(2*np.pi/self.period*(
            X[:, None, :] - X2[None, :, :]))/(2.*np.square(self.lengthscale)*self.period)).sum(1)
        return dX
