            ...
            for fundamental_frequency in frequencies:
                for m in range(M):
                    cosine_series += np.cos((m + 1) * 2 * np.pi *
                                            fundamental_frequency * np.linalg.norm(x1/sample_rate - x2/sample_rate))

            kernel_value = (1 / np.pi) * np.exp(-(sigma**2 / 2)
                                                * np.linalg.norm(x1/sample_rate - x2/sample_rate)**2) * cosine_series
...
def plot_frequency_response(data, data2, sample_rate):
    ...
    if data2 is not None:
        ...
        N2 = len(data2)
        normalise2 = N2/2
        frequency_axis2 = fftfreq(N2, 1.0/sample_rate)
        norm_amplitude_2 = np.abs(fft_data2)/normalise2
        plt.plot(frequency_axis2[:N2//2], norm_amplitude_2[:N2//2], 'r')
    # Plot the results
...
Y_train = data[:no_samples].reshape(-1, 1)
...
X = np.linspace(0, time_length, no_samples).reshape(-1, 1)


# Plot frequency response of prior covariance
X = np.linspace(-50, 50, 100).reshape(-1, 1)
Y = np.zeros(len(X)).reshape(-1, 1)
prior_cov = MoG_spectral_kernel_matrix(X, Y)
print(prior_cov)
plt.plot(X, prior_cov)
plt.show()

# Convert audio data to frequency domain
fft_data = fft(prior_cov)
N = len(prior_cov)
normalise = N/2
