...
class CustomSpectralMixtureKernel(Kernel):
    r"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SpectralMixtureKernel, self).__init__(
            ard_num_dims=ard_num_dims, batch_shape=batch_shape, **kwargs)
        ...
        self.register_constraint(
            "raw_mixture_weights", mixture_weights_constraint)
    ...
    def _set_mixture_scales(self, value: Union[torch.Tensor, float]):
        ...
        self.initialize(
            raw_mixture_scales=self.raw_mixture_scales_constraint.inverse_transform(value))
    ...
    def _set_mixture_means(self, value: Union[torch.Tensor, float]):
        ...
        self.initialize(
            raw_mixture_means=self.raw_mixture_means_constraint.inverse_transform(value))
    ...
    def _set_mixture_weights(self, value: Union[torch.Tensor, float]):
        ...
        self.initialize(
            raw_mixture_weights=self.raw_mixture_weights_constraint.inverse_transform(value))
    ...
    def initialize_from_data_empspect(self, train_x: torch.Tensor, train_y: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            slopes = (spec_cdf[bins] - spec_cdf[bins - 1]) / \
                (freq[bins] - freq[bins - 1])
            ...
            GMM = GaussianMixture(
                n_components=self.num_mixtures, covariance_type="diag").fit(inv_spec)
            means = GMM.means_
            ...
            self.mixture_means = torch.tensor(
                means, dtype=dtype, device=device).unsqueeze(-2)
            self.mixture_scales = torch.tensor(
                varz, dtype=dtype, device=device).unsqueeze(-2)
            self.mixture_weights = torch.tensor(
    ...
    def initialize_from_data(self, train_x: torch.Tensor, train_y: torch.Tensor, **kwargs):
        ...
        with torch.no_grad():
            ...
            dists = torch.where(dists.eq(0.0), torch.tensor(
                1.0e10, dtype=train_x.dtype, device=train_x.device), dists)
            ...
            self.mixture_scales = torch.randn_like(
                self.raw_mixture_scales).mul_(max_dist).abs_().reciprocal_()
            # Draw means from Unif(0, 0.5 / minimum distance between two points)
            self.mixture_means = torch.rand_like(
                self.raw_mixture_means).mul_(0.5).div(min_dist)
    ...
    def forward(
        ...
        x1_exp_, x2_exp_ = self._create_input_grid(
            x1_exp, x2_exp, diag=diag, **params)
        x1_cos_, x2_cos_ = self._create_input_grid(
            x1_cos, x2_cos, diag=diag, **params)
        ...
        mixture_weights = self.mixture_weights.view(
            *self.mixture_weights.shape, 1, 1)
        if not diag:
